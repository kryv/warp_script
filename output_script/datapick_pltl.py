...
if True :
 ...
 for jj in [0,50,100]:
  for ii in ['U32','U33','U34']:
   ...
   px1.set_xlabel('$x$ [mm]')
   px1.set_ylabel('$y$ [mm]') 
   px1.set_title('$x-y$ space')
   px1.set_xlim(pxmin-0,pxmax-0)
   px1.set_ylim(pymin+20,pymax-20)

   px2 = apm.msubplot(222)
   px2.set_xlabel('$y$ [mm]')
   px2.set_ylabel('$y\'$ [mrad]') 
   px2.set_title('$y-y\'$ phase space')
   px2.set_xlim(pxmin+20,pxmax-20)
   px2.set_ylim(pymin+20,pymax-20)
   
   px3 = apm.msubplot(223)
   px3.set_xlabel('$x$ [mm]')
   px3.set_ylabel('$x\'$ [mrad]') 
   px3.set_title('$x-x\'$ phase space')
   px3.set_xlim(pxmin-0,pxmax-0)
   px3.set_ylim(pymin+20,pymax-20)
   
   px4 = apm.msubplot(224)
   px4.set_xlabel('$x\'$ [mrad]')
   px4.set_ylabel('$y\'$ [mrad]') 
   px4.set_title('$x\'-y\'$ phase space')
   ...
   cdvxvy = getcdata(vx,vy)
   cdyvx = getcdata(yy,vx)
   

   pltint = 1
   #if jj == 0:
   obj=apm.splot(px1,xx[::pltint],yy[::pltint],c=cdxy[::pltint],s=1,lw=0)
   apm.cbars(obj)
   apm.splot(px2,yy[::pltint],vy[::pltint],c=cdxy[::pltint],s=1,lw=0)
   apm.splot(px3,xx[::pltint],vx[::pltint],c=cdxy[::pltint],s=1,lw=0)
   apm.splot(px4,vx[::pltint],vy[::pltint],c=cdvxvy[::pltint],s=1,lw=0)
