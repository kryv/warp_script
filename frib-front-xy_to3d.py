...
vvv = 1.880390784730036824e-03    # r offset of the bend
www = 1.223855502462149834e-03    # B field scale factor of the bend
...
# --- Set same envelope size for all species with various emittance 
U_r_x = zeros(U_ns)
U_r_y = zeros(U_ns)
U_rp_x = zeros(U_ns)
U_rp_y = zeros(U_ns)
...
if sim_area == 0 :
    s4p1_str = 0.6 #0.6 # 0.754 # S4 1: peak on-axis B_z field strength [Tesla]
elif sim_area == 1 :
    s4p1_str = 0.0
...
if sim_area == 0 :
    s4p2_str = 0.5 #0.5 # 0.617 # s4 2: peak on-axis B_z field strength [Tesla]
elif sim_area == 1 :
    s4p2_str = 0.0
...
if sim_area == 0 :
    gag_typ = "lin"       # Grated Accel Gap: type: "ideal" = Short gap kick, "lin" = linear r-z field imported, "nl" = nonlinear r-z field imported   
elif sim_area == 1 :
    gag_typ = "no"
...
if fi.gag_nz != gag_nz:
    raise Exception("GAG: Nonlinear and linear field model nz not equal") 
gag_nr = fi.gag_nr
gag_dr = fi.gag_dr
gag_r_m = fi.gag_r_m
...
d5a_cen = d5_tmp_s + 90*cm + vvv/sqrt(2.0)
d5a_s = d5a_cen - d5a_zlen/2.0
d5a_e = d5a_cen + d5a_zlen/2.0
d5a_xs = - d5_tmp_rc-15*cm/sqrt(2.0) 
...
if d5p1_typ == "nl" :
    ...
    if sim_area == 1 :
        data = getdatafromtextfile(bnd_grid_tr,dims=[3,None],)
        if len(data[0]) != (d5a_nx+1)*(d5a_ny+1)*(d5a_nz+1):
            raise Exception("bend grid data is invalid.")
        d5a_bx = data[0].copy()
        d5a_by = data[1].copy()
        d5a_bz = data[2].copy()
    ...
    d5a_3d_id = addnewbgrddataset(dx=d5a_dx, dy=d5a_dy, 
                    zlength=d5a_zlen, bx=d5a_bx, by=d5a_by, bz=d5a_bz)
...
if d5p1_typ == "lin":
    ...
    d5p2 = addnewbgrd(dx=d5a_dx, dy=d5a_dy, xs=d5a_xs, ys=-0.5*d5a_yw,
                      ...
                      he=0, lb=0, ot = -pi*0.25,oy = d5a_oy, op = 0.0,
                      ph = d5a_ph)
else:
...
neut_f1 = 75.0/100.0                 # corresponding electron neutralization factors 
neut_f2 = 75.0/100.0  
...
if sim_area == 1:
    ...
    def injectall():
        for pcnt,ii in enumerate(sort(sp.keys())):
            ...
            sp[ii].addparticles(
                x=allspx[pcnt], y=allspy[pcnt], z=allspz[pcnt],
...
z_adv = 69.2 - d5sim_frng_l  # Axial position in lattice to advance to 
...
if sim_area == 1:
    diag_part_step = arange(0,4001,50)
...
def diag_calls():
  if top.it in diag_part_step:
    diag_part(plt_xy=0,plt_xxp=0,plt_yyp=0,plt_xpyp=0,
              plt_trace=false,plt_denxy=false,plt_denr=false)
  if top.it in diag_field_step: 
    diag_field(plt_pc=true,plt_pc_xy=true,plt_pa=true)
  if top.it in diag_hist_step:
    diag_hist(plt_ekin=true,plt_spnum=true,plt_curr_e=true,
              plt_curr_p=true,plt_lam_p=true,plt_lam_e=true,
              plt_lz=true,plt_pth=true,plt_pthn=true,plt_krot=true,
              plt_lang=true,plt_cen=true,plt_envrms=true,
              plt_envmax=true,plt_envrmsp=true,plt_emit=true,
              plt_emitn=true,plt_emitg=true,plt_emitng=true,
              plt_emitr=true,plt_emitnr=true,plt_emitpv=true,emitpvn=true)
...
if sim_area == 0:
    ...
    n_step = nint((z_adv-top.zbeam)/wxy.ds)   # add two extra steps in case of roundoff accumulation 
    top.prwall = r_p_down    # consistent aperture 
    step(n_step)
 
    execfile( sfld + "outparticle.py")
    step(1)
    execfile( sfld + "outparticle.py")
    step(6) # add two extra steps in case of roundoff accumulation 
    execfile( sfld + "outputdat2.py")
    execfile( sfld + "outputdat3.py")
