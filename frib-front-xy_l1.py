...
for ii in range(U_ns):
 ...
 s.beta_x = tmp_beta_x
 U_r_x[ii] = sqrt(emit_edge*tmp_beta_x)
 U_r_y[ii] = sqrt(emit_edge*tmp_beta_y)
 U_rp_x[ii] = -sqrt(emit_edge/tmp_beta_x)
 U_rp_y[ii] = -sqrt(emit_edge/tmp_beta_y)
 #print("    Species: " +ii+" beta_x,y = %s"%tmpbeta_x)
...
for ii in range(O_ns):
 ...
 tmp_beta_y = tmp_beta_x
 s.beta_x = tmp_beta_x
 O_r_x[ii] = sqrt(emit_edge*tmp_beta_x)
 O_r_y[ii] = sqrt(emit_edge*tmp_beta_y)
 O_rp_x[ii] = -sqrt(emit_edge/tmp_beta_x)
 O_rp_y[ii] = -sqrt(emit_edge/tmp_beta_y)
...
# --- --- linear element data  
fi = PRpickle.PR("/home/k/wfrib/test/lat_ecr_venus.lin.20140602.pkl")
